...
function drawingGameBoard(callback, ctx, img) {
  const promise = new Promise((resolve) => {
    drawingGameField(1, ctx);
    drawingGameField(2, ctx);
    encodPNGToStream(callback, resolve, img);
  });
  return promise;
}

function drawingGoodShot(callback, i, j, ctx, img) {
  const promise = new Promise((resolve) => {
    i -= 1;
    j -= 1;
    ctx.beginPath();
...
function drawingBadShot(callback, i, j, ctx, img) {
  const promise = new Promise((resolve) => {
    ctx.beginPath();
    ctx.arc(300 * i, 300 * j, 50, 0, Math.PI, false);
    ctx.arc(300 * i, 300 * j, 50, 0, Math.PI, true);
    ctx.closePath();
    ctx.fill();
    encodPNGToStream(callback, resolve, img);
  });
...
function drawingAllShips(array, ctx, img) {
  const promise = new Promise((resolve) => {
    for (let i = 1; i < 9; i++) {
      for (let j = 1; j < 9; j++) {
    ...
    encodPNGToStream(null, resolve, img);
  });
  return promise;
}
